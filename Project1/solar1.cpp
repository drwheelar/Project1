...
void updataWithInput(bulletchain** bullet, item* a, item* b, item* c, item* d, item* e,double*shootrad,int*stop,
	double speed1, int size1, int attack1, int* oxygengain,int* carbongain, int* irongain, int* silicongain,
	IMAGE *bulletimg);
int show(IMAGE* rubbishpic, IMAGE* cover, bulletchain* bullet, item* oxygen, item* carbon, item* iron,
	...
	int* silicongain, char exception[],int time);
void solar1(IMAGE* rubbishpic, char exception[], double speed1, int size1, int attack1, int time1,
	int*oxygengain,int*carbongain, int*irongain, int*silicongain) {
	...
	while (1) {
		...
		updataWithoutInput(&bullet,&oxygen,&carbon,&iron,&silicon,&rubbish,&oxygennumber,&carbonnumber,
			&ironnumber,&siliconnumber,&rubbishnumber,oxygengain,carbongain,irongain,silicongain,
			exception);
		updataWithInput(&bullet, oxygen, carbon, iron, silicon, rubbish, &shootrad,& stop, speed1, size1, attack1,
			oxygengain,carbongain, irongain, silicongain,bulletimg);
		timerest=show(rubbishpic, cover, bullet, oxygen, carbon, iron, silicon, rubbish, after, shootrad, oxygengain, 
			carbongain,irongain, silicongain, exception,timestart);
		if (timerest <= 0) break;
	}
}
